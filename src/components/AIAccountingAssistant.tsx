  ...
  const [messages, setMessages] = useState<AssistantMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: 'Hello! I am the built-in Herrera Accounting Bot. Ask me about today’s entries, missing receipts, overdue AP, collections, approvals, or cash position. Type “help” for examples.',
      timestamp: new Date()
    }
  ]);
    ...
    <div className="bg-white border border-slate-200 rounded-xl flex flex-col overflow-hidden h-[500px]">
      <div className="p-4 border-b border-slate-200 bg-white flex items-center justify-between">
        <h3 className="text-slate-900 text-sm font-mono uppercase tracking-widest flex items-center gap-2">
          ...
          Herrera Accounting Bot
        </h3>
        <span className="rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-mono font-bold uppercase tracking-wider text-emerald-700">
          Local • No API
        </span>
      </div>
      ...
      <div className="p-3 bg-white border-t border-slate-200">
         ...
         <form onSubmit={(e) => handleSend(e)} className="relative flex items-center">
           ...
           <button
              ...
              className="absolute right-1.5 p-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-50 disabled:text-slate-500 text-white rounded transition-colors"
           >
